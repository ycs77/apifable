...
apifable 是一套深度探索 OpenAPI 規格並從中生成程式碼的工具包。它以 MCP 伺服器的形式打造，為 AI 代理提供讀取、搜尋和理解任何 OpenAPI 規格的工具。搭配 Recipe（一個定義專案程式碼慣例的風格指南 `.md` 檔案），以及內建的 `apifable-codegen` 技能，即可將規格資料轉換為具備型別的可用程式碼。
...
apifable 內建一個 Agent Skill——`apifable-codegen`——將 MCP 伺服器與 Recipes 整合為完整的程式碼生成工作流程：

1. **找到 Recipe** — 讀取 `.apifable/recipes/` 中已安裝的風格指南；若沒有合適的，會建議執行 `recipe add`
